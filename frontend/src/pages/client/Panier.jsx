...
const ZONES = [
  { id:1, name:"Centre-ville", max:2, price:500 },
  { id:2, name:"Quartier Nord", max:5, price:1000 },
  { id:3, name:"Quartier Sud", max:10, price:1500 },
  { id:4, name:"Périphérie", max:20, price:2500 },
  { id:5, name:"Zone rurale", max:40, price:4000 },
];
...
export default function Panier() {
  ...
  const grandTotal = total + commission + zone.price;
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      nav("/client/suivi?order=" + order.id);
    } catch (err) {
  ...
  if (count === 0) return (
    <div className="min-h-screen flex flex-col items-center justify-center text-gray-400 px-4">
      ...
      <button onClick={() => nav("/client/catalogue")}
  ...
  return (
    <div className="pb-32 px-4 py-4">
        ...
        <div className="grid grid-cols-3 gap-2">
          {ZONES.map(z => (
            <button key={z.id} onClick={() => setZone(z)}
              ...
              <div className={`text-sm font-bold ${zone.id===z.id?"text-emerald-600":"text-gray-600"}`}>{z.price.toLocaleString("fr-FR")} F</div>
              <div className="text-xs text-gray-400">≤{z.max} km</div>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 mb-4">
        ...
        {[["Sous-total", total],["Commission (10%)", commission],["Livraison", zone.price]].map(([k,v])=>(
          <div key={k} className="flex justify-between py-1 text-sm text-gray-500">
            <span>{k}</span><span>{v.toLocaleString("fr-FR")} F</span>
          </div>
        ))}
